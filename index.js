...
const broadcast = (type, message) => {
  ...
  for(key in clients) {
    if (clients[key]) {
      clients[key].send(JSON.stringify({ message: message, type: type }));
    }
  }
}
...
wsServer.on('connection', (connection) => {
  ...
  connection.send(JSON.stringify({ type: 'client-connected', userID: userID }));
  ...
  connection.on('message', (message) => {
    ...
    if (type === 'disconnect-player') {
      clients[value.userID].send(JSON.stringify({ type: 'you-were-disconnected' }));
